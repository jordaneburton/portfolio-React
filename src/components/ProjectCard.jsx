...
function ProjectCard({ urlLinks, thumbnail, title }) {
    ...
    return (
        <>
            <Col xs={12} md={6} lg={4} className='project-card position-relative z-2 my-5'>
                <Image src={thumbnail} thumbnail role='img' aria-label='Project thumbnail'/>
                <svg onClick={() => { openRepo() }}
                    onMouseEnter={handleMouseEnterGit}
                    onMouseLeave={handleMouseLeaveGit}
                    style={{ 
                        mixBlendMode: isHoveredGit ? 'difference' : 'normal', 
                        fill: isHoveredGit ? 'white' : '#6709FF'
                    }}
                    xmlns="http://www.w3.org/2000/svg" width="6rem" height="6rem" className="bi bi-github position-absolute top-50 start-50 translate-middle" viewBox="0 0 16 16" role="link" aria-label="Github link">
